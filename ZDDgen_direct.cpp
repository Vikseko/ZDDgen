...
int main (int argc, char *argv[])
{
    ...
    for (int i = 0; i < learnts.size(); ++i) //hash_table
    {
      for (int j = 0; j < learnts[i].size(); ++j)
      {
        auto it = clause_hasher.find(abs(learnts[i][j]));
        if(it != clause_hasher.end())
        {
          it->second++;
        }else{
          clause_hasher[abs(learnts[i][j])] = 1;
        }
      }
    }

    std::vector<std::pair<int, int> > mapcopy(clause_hasher.begin(), clause_hasher.end());
    std::sort(mapcopy.begin(), mapcopy.end(), less_first<int, int>()); // sorting vars 

    std::vector<std::pair<int,int>> mapvars ; //map for renaming vars
    ...
    for (int j = 0; j < learnts.size(); ++j) //making sets
    {
      ...
      for (int i = 1; i <= 2*mapvars.size(); i++){  //literal addition
        if(!(std::find(learntemp.begin(), learntemp.end(), i) != learntemp.end()) && !(std::find(learntemp.begin(), learntemp.end(), (-1)*i) != learntemp.end())) {
          learntemp.push_back(i*(-1));
        }
      }
    ...
    gbm = Cudd_Init(0,2*mapvars.size(),CUDD_UNIQUE_SLOTS,CUDD_CACHE_SLOTS,0); // Initialize a new BDD manager. 
    ...
    tzddstart = clock();
    std::cout << " Start creating zdd vars " << std::endl;
    crzddvarsstart = clock();
    std::vector<DdNode *> zddvars (2*mapvars.size());
    for (int i = 0; i < 2*mapvars.size(); ++i)
    {
      zddvars[i] = Cudd_zddIthVar(gbm, i);
      Cudd_Ref(zddvars[i]);
    }
    std::cout << " Start creating complements of zdd vars " << std::endl;
    crnegzddvarsstart = clock();
    std::vector<DdNode *> zddnegvars (2*mapvars.size());
    for (int i = 0; i < 2*mapvars.size(); ++i)
    {
    ...
    crzddvarsend = clock();
    std::cout << " Start creating ZDD " << std::endl;
